...
def parse_args():
    ...
    parser.add_argument(
        "--input_json_folder",
        type=str,
        default="/storage/hxy/ID/data/data_processor/sample/step2/istockv4",
        help="Path to the folder containing MP4 files.",
    )
    parser.add_argument(
        "--output_json_folder",
        type=str,
        default="/storage/hxy/ID/data/data_processor/test/wrong_examples/frames_gm621059908",
        help="Directory for output files.",
    )
    parser.add_argument(
        "--video_root", type=str, default="/storage/dataset/istock/", help="Directory for output files."
    )
    ...
    parser.add_argument(
        "--api_key",
        type=str,
        default="da36c8ba202714f3c663577bf4c10c63.jfXshFLsTaHBavmz",
        help="Directory for output files.",
    )
...
def cap_video(video_base, API):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "user",
                "content": [{"type": "video_url", "video_url": {"url": video_base}}, {"type": "text", "text": temp}],
            }
        ],
        temperature=0.1,
        max_tokens=512,
...
def process_json_file_test(json_file, input_json_folder, output_json_folder, video_root, API):
    ...
    if metadata["cut"] == metadata["face_cut"]:
        json_data["metadata"]["face_cap_glm"] = []
...
def process_json_file(json_file, input_json_folder, output_json_folder, video_root, API):
    ...
    num_frames = 16
    temp_video_path = json_file.replace(".json", ".mp4")
    video_base64 = save_frames_to_video_and_get_base64(crop_frames, temp_video_path, num_frames, fps)

    video_caption = cap_video(video_base64, API)
    json_data["metadata"]["face_cap_glm"] = [video_caption.content]
