...
def load_video_with_cut(video_file, cut=None, crop=None):
    ...
    def resize_longest_edge(image, length):
        w, h = image.size
        if w > h:
            new_w, new_h = length, int(length * h / w)  # 宽 > 高
        else:
            new_w, new_h = int(length * w / h), length  # 高 > 宽
        return image.resize((new_w, new_h), Image.LANCZOS)  # **修正 LANCZOS 访问方式**

    resized_images = [resize_longest_edge(to_pil_image(frame),512) for frame in video_frames]

    frames = [to_pil_image(frame) for frame in video_frames]
...
class CaptionData(Dataset):
    def __init__(self, video_data, video_root, save_root, processor, model):
        super().__init__()
...
def parse_args():
    ...
    # parser.add_argument("--input_json", type=str, default="/storage/hxy/ID/data/data_processor/sample/step4/Aria_test/istockv4_sample.json")
    # parser.add_argument("--video_root", type=str, default="/storage/dataset/istock/")
    # parser.add_argument("--save_root", type=str, default="/storage/hxy/ID/data/data_processor/sample/step4/Aria_test/cap_transformer_0.9")
    ...
    parser.add_argument("--save_root", type=str, default="/storage/hxy/ID/data/data_processor/sample/step4/cap_test/cap_jsons")
...
if __name__ == "__main__":
    ...
    # model_id_or_path = "/storage/ysh/Ckpts/Aria-torchao-int8wo"
    ...
    from transformers import AriaProcessor, AriaForConditionalGeneration, AutoModelForCausalLM, AutoProcessor
    ...
    model = AriaForConditionalGeneration.from_pretrained(model_id_or_path, torch_dtype=torch.bfloat16, device_map="auto")
    # model = AriaForConditionalGeneration.from_pretrained(model_id_or_path, torch_dtype=torch.bfloat16, trust_remote_code=True).to('cuda')


    processor = AriaProcessor.from_pretrained(model_id_or_path)
    
    # model = AutoModelForCausalLM.from_pretrained(model_id_or_path, torch_dtype=torch.bfloat16)
    ...
    data = CaptionData(video_data, args.video_root, args.save_root, processor, model)
    ...
    for save_paths, frames, video_key in tqdm(loader):
        ...
        try:
            ...
            for inputs in frames:
                ...
                # save_path_test = save_paths[0].replace('.json','_test.json')
